...
| | | — **AU** (Audit): Drives the entire result storage architecture — append-only `GauntletProbeResult` CRs (AU-9), HMAC result signing (AU-10), SIEM export (AU-4, AU-11), 365-day retention floor (AU-11), fail-closed on export failure (AU-5) |
...
| **NIST SP 800-53A Rev 5** (Assessing Security and Privacy Controls) | The assessment procedures companion to 800-53. Defines how to evaluate whether controls are implemented correctly and operating effectively. | Drives the SAP template (`compliance/sap/`). Every SAP test procedure maps to an 800-53A assessment objective. Gauntlet's continuous probe execution replaces point-in-time assessment for covered controls — the assessor reviews the probe result record instead of re-running the tests manually. |
| **NIST SP 800-37 Rev 2** (Risk Management Framework) | Defines the RMF lifecycle: Categorize → Select → Implement → Assess → Authorize → Monitor. | Gauntlet operates in the **Monitor** step. It produces the continuous monitoring evidence that keeps an ATO current between annual assessments. `GauntletIncident` CRs feed directly into POA&M entries for the **Assess** step. |
| **NIST SP 800-137** (Information Security Continuous Monitoring) | Defines the continuous monitoring strategy: what to monitor, how often, and how to respond to findings. | The **operational mandate** Gauntlet fulfills. Probe scheduling intervals are tied to FIPS 199 impact level per 800-137 guidance (High: every 6 hours, Moderate: every 24 hours). The continuous monitoring strategy in the SSP template (`compliance/ssp/` section 21) documents Gauntlet's monitoring cadence per control family. |
| **NIST SP 800-207** (Zero Trust Architecture) | Defines zero trust principles: verify explicitly, least privilege, assume breach. | Gauntlet applies ZTA to itself — the controller is a protected resource with per-request authentication, explicit egress NetworkPolicy, and blast radius controls assuming controller compromise. The per-probe ServiceAccount model is least-privilege by design. |
...
| **NIST SP 800-171 Rev 3** (Protecting CUI in Nonfederal Systems) | Controls for protecting Controlled Unclassified Information (CUI) — required for DoD contractors. | Gauntlet's control coverage overlaps substantially with 800-171 requirements. Agencies using Gauntlet on CUI systems get continuous validation of 800-171 controls through the same probe mechanisms. |
| **NIST SP 800-218** (Secure Software Development Framework, SSDF) | Practices for secure software development lifecycle. | Governs Gauntlet's own development practices: CI/CD pipeline with CVE scanning (Trivy, govulncheck), dependency management (Dependabot), SBOM generation, and SLSA provenance attestations. |
...
| Standard | What It Is | How It Shapes Gauntlet |
|---|---|---|
| **FedRAMP** (Federal Risk and Authorization Management Program) | The government-wide program for standardizing security assessment and authorization of cloud services. Builds on NIST 800-53 with additional FedRAMP-specific requirements. | Drives several specific requirements: 365-day minimum in-cluster audit retention (AU-11), 3-year SIEM retention, 30-day Critical CVE remediation SLA, OSCAL-formatted ATO artifacts, and the Customer Responsibility Matrix (`compliance/crm/`). Gauntlet's Phase 1 compliance package is structured for a FedRAMP ATO. |
...
| Standard | What It Is | How It Shapes Gauntlet |
|---|---|---|
| **MITRE ATT&CK for Containers** | Adversary behavior taxonomy specific to container environments. | Every `GauntletProbe` maps to a MITRE technique ID. The detection probe's approved syscall catalog is organized by ATT&CK technique. This mapping makes Gauntlet's output directly usable in threat-informed defense reporting. |
| **OSCAL** (Open Security Controls Assessment Language) | NIST's machine-readable format for security control documentation. | Gauntlet's ATO package uses OSCAL as the foundation format. The 40 control implementation narratives compile from Trestle markdown to OSCAL JSON. `GauntletProbeResult` records are structured to map to OSCAL `assessment-results` findings. |
| **SLSA** (Supply-chain Levels for Software Artifacts) | Framework for supply chain integrity, from Google. Levels 1–4. | Gauntlet targets **SLSA Level 2** — each image has a provenance attestation linking it to the source commit and CI build run. Provenance is cosign-attested and Rekor-logged. |
...
### How Standards Map to Gauntlet Components

| Gauntlet Component | Primary Standards |
|---|---|
...
Each of these is an ATO evidence gap — a period where the system's actual security posture diverged from its documented controls. Gauntlet surfaces these regressions in near-real-time and records them in an immutable, SIEM-exported audit trail.

---

## Deployment Profiles and Abstract Capabilities

Gauntlet is opinionated about *what* security capabilities must be present — but flexible about *which tools* provide them. The operator selects a deployment profile that binds Gauntlet's abstract capability requirements to the concrete tools installed in the cluster. Gauntlet adjusts its policy templates, backend connections, and compliance documentation output accordingly.

### Abstract Capabilities

Gauntlet requires five external capabilities. Each has a defined interface contract and one or more supported implementations.

| Capability | What Gauntlet Needs | Interface Contract | Supported Implementations |
|---|---|---|---|
| **Admission Enforcement** | Enforce 4 policies at Pod/resource admission: image signature verification, audit record immutability, Job SA constraints, no writable PVC | Kubernetes admission webhook that evaluates and enforces policy rules shipped with the Gauntlet Helm chart | Kyverno (ClusterPolicy), OPA/Gatekeeper (ConstraintTemplate + Constraint) |
| **Image Signature Verification** | Verify cosign signatures on all Gauntlet container images before Pod admission | Subset of admission enforcement — the admission controller must support cosign signature verification | Kyverno (cosign verifyImages), Sigstore policy-controller, Connaisseur |
...
The following defines the Gauntlet authorization boundary for ATO package purposes.

**Inside the boundary:**
- Controller Manager (Go operator)
- Probe Runner Jobs (ephemeral)
- `GauntletProbe`, `GauntletProbeResult`, `GauntletAOAuthorization`, `GauntletSystemAlert`, `GauntletIncident` CRDs
- `gauntlet-system` namespace and all resources within it
...
Each external connection crosses the Gauntlet authorization boundary. Per NIST 800-53 CA-3, the deploying agency must execute an Interconnection Security Agreement (ISA) with the owner of each external system. Gauntlet's design documents the connection parameters (direction, data type, protocol, security controls) required to complete those agreements — the agency executes them.

---

## Authentication for External Systems

Gauntlet authenticates to all external systems using individual, rotatable credentials. No anonymous or shared credential access is permitted.
...
Under NIST 800-207, Gauntlet is itself a protected resource — not merely a security tool. A compromised controller holds ATO evidence, can create Jobs with adversarial capabilities, and has connections to detection and network enforcement backends. The following ZTA controls apply to Gauntlet as a resource:

- All access to Gauntlet APIs (CRD reads/writes) is authenticated per-request via Kubernetes RBAC. No persistent privileged sessions.
...
- Gauntlet RBAC role assignments are reviewed quarterly by the ISSO. Access reviews are documented and retained as AU evidence.
...
All Gauntlet operator accounts must be individual (not shared service accounts) to satisfy AU-10 non-repudiation requirements. The following roles are defined:
...
Controller queues probe execution
  ...
  → Controller writes GauntletProbeResult (append-only)
  → Job TTLs out and is garbage collected
```

The controller-manager holds a completely separate ServiceAccount with permissions only to create and watch Jobs and read/write `GauntletProbe` and `GauntletProbeResult` resources. It never holds probe-class permissions directly. If the controller is compromised, it cannot itself execute probes.
...
**`--dry-run=server` edge case**: Some admission webhooks are configured to skip dry-run requests. The probe detects this condition — if the API server response does not include evidence of webhook evaluation, the outcome is `Indeterminate` (not `Rejected`), preventing a false positive. The `GauntletProbeResult` detail field documents which webhooks were evaluated.
...
**Default (Option A) — Gauntlet-shipped synthetic policy**
The Helm chart installs a lightweight admission policy (rendered for the configured admission controller — Kyverno ClusterPolicy or OPA Constraint) that rejects any resource carrying the label `gauntlet.io/admission-probe: "true"`. The probe always submits a resource with this label. Gauntlet controls both sides — the policy and the probe spec — making the test deterministic and requiring zero operator configuration.

**Override (Option B) — Operator-configured policy reference**
For federal environments with mandated policy sets, operators can override the default by specifying a target policy and a known-bad resource spec in `GauntletProbe`:
...
- The controller queries the CNI observability API for flow records matching the probe's `gauntlet.io/probe-id` label
...
Gauntlet deploys a lightweight responder pod in the target namespace that always responds on a known port. The probe sends traffic to the responder. If the response arrives, traffic was forwarded. If it times out, it was dropped. This eliminates the "destination not running" ambiguity from tcp-inference.

- Requires: Gauntlet must be able to create pods in the target namespace (expanded RBAC for the `gauntlet-probe-netpol` ServiceAccount)
...
Every `GauntletProbeResult` for a NetworkPolicy probe includes `spec.result.verificationMode` (`cni-verdict`, `tcp-inference`, or `responder`) so the assessor knows exactly what level of evidence the result represents.

### Alert Correlation

The probe Job pod carries the label `gauntlet.io/probe-id: <uuid>`. In `cni-verdict` mode, CNI observability layers enrich flow records with Kubernetes pod metadata, causing this label to appear on the flow record. The controller queries for flow records matching the probe ID to establish correlation — identical to the detection probe correlation mechanism.
...
Every `GauntletProbe` of type Detection must reference a `GauntletAOAuthorization` resource via `spec.aoAuthorizationRef`. Detection probe Jobs are not created if the referenced authorization is missing or expired. The `GauntletAOAuthorization` resource captures:
...
`GauntletAOAuthorization` resources are append-only and exported to the SIEM audit log. Authorization expiration surfaces as a `GauntletSystemAlert` requiring acknowledgment before detection probes resume.
...
The active catalog version is referenced in the `GauntletAOAuthorization` resource and included in the SSP. Catalog updates require a new AO authorization.
...
Every probe Job pod carries the label `gauntlet.io/probe-id: <uuid>`. Falco and Tetragon both enrich alerts with Kubernetes pod metadata, causing this label to appear on any alert generated by activity inside the probe container. The controller queries the detection backend for alerts carrying the matching probe ID to establish correlation.

Cluster operators must not add exceptions to Falco or Tetragon rules that suppress alerts from pods carrying `gauntlet.io/*` labels. Doing so would cause detection probes to return `Undetected` while leaving the same gap open for real attackers.
...
|---|---|---|
| RBAC | `gauntlet-probe-rbac` | Attempt actual operations (e.g., `GET` a secret outside authorized namespace) and verify 403 rejection — not `can-i`. Tests: RoleBinding creation, impersonation, pod exec access, cross-namespace resource reads |
| NetworkPolicy | `gauntlet-probe-netpol` | None — relies on CNI observability layer for flow verdicts, not API access |
...
**Probe Fingerprinting** — every probe Job pod carries the label `gauntlet.io/probe-id: <uuid>` and all API calls carry a `gauntlet.io` user-agent. For detection probes, the pod label serves as the correlation key for alert matching (see Detection Probe Verification). This label is the fingerprint — it does not suppress detection, it identifies the source after the fact. A probe action without a valid fingerprint does not execute.
...
Probe image signing keys and any cryptographic keys used within Gauntlet are governed per NIST 800-53 SC-12.
...
- **Distribution**: Public verification key shipped with the Gauntlet Helm chart. For air-gapped environments, key custody transfer follows a documented procedure included in the Helm chart deployment guide.
...
- **Sigstore Rekor transparency log** — every cosign signing event (signatures and attestations) is published to the Rekor append-only transparency log; Rekor is external to Gauntlet infrastructure, publicly queryable, and provides independently verifiable non-repudiation of all build and signing events

### Admission Enforcement

An admission enforcement policy shipped with the Helm chart (rendered for the configured admission controller) verifies the cosign signature of every probe runner image before the Job pod is admitted. Any Job referencing an image without a valid Gauntlet signature is rejected at admission — it never runs. This gate is independent of the controller: even a compromised controller cannot create a Job with an unsigned or tampered image.
...
2. Creates a `GauntletSystemAlert` resource with `reason: AdmissionPolicyMissing`
3. Surfaces a `DegradedMode` condition on its own status
4. Emits a Prometheus metric (`gauntlet_bootstrap_verification_status`) for alerting

`GauntletSystemAlert` resources are used throughout Gauntlet to surface degraded states. Each alert includes `spec.reason` (enum), `spec.message`, `spec.acknowledged` (bool), `spec.acknowledgedBy` (Kubernetes username), `spec.acknowledgedAt`, and `spec.remediationAction`. The full set of alert reasons is:
...
| `AuditWriteFailure` | Failed to write GauntletProbeResult to etcd |
...
| `AOAuthorizationExpired` | Active GauntletAOAuthorization has passed its `expiresAt` time |
...
Probe execution does not resume until an authorized operator explicitly acknowledges the `GauntletSystemAlert`. The acknowledging principal must be an individual Kubernetes RBAC identity (not a shared service account) — shared service account acknowledgments are rejected at admission. The acknowledgment record captures: Kubernetes username from request context, timestamp, and stated remediation action. This satisfies AU-10 non-repudiation requirements and serves as a POA&M evidence record under NIST 800-53 CA-7 and SI-7.

---

## Result Storage and Audit Log

Gauntlet is designed for Federal systems operating under continuous monitoring requirements (FedRAMP, FISMA, NIST 800-53). Result storage uses a two-tier model that separates operational visibility from the authoritative audit record.

### Tier 1 — Operational View

Probe results are written to a status subresource on `GauntletProbe`. This holds the latest result, the last N executions (default 10, configurable), and a `consecutiveFailures` counter used for alerting. This tier is mutable and intended for day-to-day operator visibility via `kubectl`. It is not the audit record.

### Tier 2 — Append-Only Audit Log

Every probe execution creates a `GauntletProbeResult` resource. These resources are:
- Namespace-scoped to `gauntlet-system`. RBAC restricts result visibility to operators of the corresponding target namespace. Cluster-wide aggregation requires explicit cluster-admin access.
- Never modified after creation — enforced by an admission enforcement policy that explicitly denies UPDATE and DELETE operations on `GauntletProbeResult` for all principals. Only CREATE is permitted.
...
The `gauntlet-audit-admin` role provides read-only access to `GauntletProbeResult`. This role is separate from cluster-admin. Per AU-9(4), the audit administrator and system administrator roles must be held by separate individuals — enforced administratively by the ISSO during account provisioning and verified through access review.
...
Control failure outcomes represent active security control gaps on a live federal system. These are incidents, not merely metrics. When a probe returns a failure outcome (`Forwarded`, `Undetected`, `Accepted`), Gauntlet:

1. Creates a `GauntletIncident` resource capturing the failure details
2. Exports the incident to a configurable IR webhook (ServiceNow, JIRA, or generic webhook endpoint)
3. Increments the `consecutiveFailures` counter on the `GauntletProbe` status, triggering Alertmanager rules

The `GauntletIncident` resource is append-only and exported to the SIEM. Each incident includes:

| Field | Type | Description |
|---|---|---|
| `spec.probeResultRef` | string | Name of the GauntletProbeResult that triggered the incident |
...
Gauntlet provides the following configuration options for environments where this data constitutes PII:
...
PII assessment, System of Records Notice (SORN) requirements, and privacy impact assessment (PIA) are the deploying agency's responsibility under the NIST 800-53 PT control family and applicable privacy law. These Gauntlet configuration options provide the technical mechanism; the agency determines applicability.

---

## Personnel Security

Positions with access to Gauntlet are security-sensitive: Gauntlet holds ATO evidence, can enable adversarial probing of federal infrastructure, and has connections to detection backends. The following personnel security requirements apply per NIST 800-53 PS controls:

- **PS-2**: Positions with `gauntlet-operator`, `gauntlet-live-executor`, or `gauntlet-approver` roles require formal risk designation
- **PS-6**: All individuals with Gauntlet access must execute a formal access agreement prior to provisioning
- **PS-7**: Contractor access to Gauntlet is subject to PS-7 requirements; contractor personnel security must be documented in the SSP

These are agency-level obligations documented in the SSP and enforced through the agency's access management procedures, not Gauntlet implementation requirements.

---

## Contingency Planning

Gauntlet is a security control — its unavailability halts ATO evidence generation. Per NIST 800-53 CP controls:

- **RTO recommendation**: 4 hours. Monitoring continuity beyond 4 hours constitutes a gap in continuous monitoring evidence.
- **RPO**: Last successful backup of `GauntletProbe` CRD instances and Helm values.
- **Backup procedure**: `GauntletProbe` configurations backed up via Velero or equivalent on the agency's defined schedule. Helm values stored in version control constitutes the configuration backup.
- **Recovery procedure**: Redeploy Helm chart from version-controlled values, restore `GauntletProbe` resources from backup. Probe execution resumes automatically after controller startup checks pass.
- **Monitoring gap documentation**: Any Gauntlet outage exceeding the RTO must be documented as a monitoring gap in the continuous monitoring report, with the duration and affected probe surfaces noted.

---

## Vulnerability Management

Gauntlet is itself subject to vulnerability management requirements per NIST 800-53 RA-5 and SI-2:
...
The deploying agency is responsible for scanning the running Gauntlet deployment per their RA-5 continuous scanning requirements.

---

## Development Security Lifecycle

Gauntlet is a security tool that validates supply chain integrity — its own development process must meet at least the standard it imposes on the systems it monitors. This section defines the security controls governing how Gauntlet is built, reviewed, signed, and released. These controls satisfy NIST 800-218 (SSDF) and SLSA Level 2 requirements.

### Source Control and Commit Integrity

**Repository**: GitHub (github.com/primaris-tech/gauntlet).
...
```
Subject: https://github.com/primaris-tech/gauntlet/.github/workflows/release.yaml@refs/tags/v1.0.0
Issuer: https://token.actions.githubusercontent.com
```

No long-lived signing key exists. The signing identity *is* the CI pipeline, verified by Fulcio. The signature and certificate are published to the Rekor transparency log. Verification checks that the image was signed by the Gauntlet release workflow from a tag on `main`:
...
All maintainers with write access to the Gauntlet repository must use:
...
Gauntlet configuration is a security-relevant artifact subject to NIST 800-53 CM controls.
...
**CM-8 (Component Inventory)**: An SBOM is generated and published with each Gauntlet release covering all Go and Rust dependencies, base images, and Helm chart dependencies. The SBOM is the CM-8 component inventory for Gauntlet.

**Helm chart integrity**: The Gauntlet Helm chart is signed with cosign and the signature verified before installation. Helm upgrade operations that modify security-relevant values require the `gauntlet-security-override` role. The Helm chart itself is subject to the same supply chain controls as probe images.

---

## Key Design Constraints

- Gauntlet itself must not be a privilege escalation vector; the controller and probe runner ServiceAccounts are strictly separated
