...
Most security tools tell you your controls are **configured**. Sidereal tells you they are **actually working** — right now, continuously, with every result mapped to the compliance frameworks your agency requires and packaged into the reports your ISSO needs.
...
**Configuration is not enforcement.** A NetworkPolicy can be defined and not enforcing. An admission webhook can be configured and silently disabled. A Falco rule can be deployed and suppressed by a config change. Between the last scan and this moment, any of these controls could have drifted — and that drift is exactly where real-world compromises happen and where ATO evidence goes stale.

**The swivel chair.** Today, an ISSO validating Kubernetes security controls must pivot between a constellation of disconnected tools — run Kubescape for posture, check Falco dashboards for detection, query the SIEM for audit records, manually crosswalk findings to NIST 800-53 (or CMMC, or CJIS, or IRS 1075), hand-build continuous monitoring reports, generate POA&M entries from spreadsheets, and package evidence for assessors. Each tool covers one piece. None of them connect the pieces. The ISSO is the integration layer — and that doesn't scale.

**Point-in-time is not continuous monitoring.** NIST 800-137 requires ongoing validation of security controls, not periodic scanning. A scan that ran at 2 AM doesn't tell you whether your controls are working at 2 PM. The gap between scans is the gap in your evidence — and your assessor knows it.

---

## What Sidereal Does

Sidereal is a Kubernetes-native operator that **actively probes** your cluster's security controls on a continuous schedule, **verifies they are operationally effective**, and **produces the compliance evidence and reports** your ISSO needs — all from a single tool.

| Probe Surface | What It Validates |
|---|---|
| **RBAC** | ServiceAccount permission boundaries are enforced — not just configured, but actually denying unauthorized operations |
...
| **Detection Coverage** | Known-bad behaviors trigger expected alerts in your detection pipeline (Falco/Tetragon) — the only open-source tool that validates the detection layer by firing a real probe and independently confirming the alert was raised |
...
Probes run on configurable schedules (default: every 6 hours for High-impact systems) with execution jitter to prevent predictable blind spots. This isn't scanning configuration state — it's firing real actions against real enforcement layers and reading real verdicts.

### Detection Layer Validation
The most differentiated capability. Sidereal fires a known-bad syscall pattern from a sandboxed container, then the controller independently queries the detection backend (Falco or Tetragon) to verify an alert was raised. Two separate identities, two separate actions. If the detection pipeline missed it, Sidereal surfaces the gap. No other open-source tool does this.

### Multi-Framework Compliance Mapping
Probe results are tagged with controls from every active compliance framework — not just NIST 800-53, but CMMC, CJIS, IRS 1075, HIPAA, and NIST 800-171. Crosswalk tables are versioned data files, not compiled code — agencies can extend or customize without rebuilding.

### Report Generation
ISSOs need deliverables, not dashboards. Sidereal generates continuous monitoring summaries, POA&M entries, control coverage matrices, executive summaries, and OSCAL-native assessment evidence packages — directly from probe results, with zero manual assembly.
...
Sidereal scans your cluster for existing security controls and generates `SiderealProbeRecommendation` resources. The ISSO's first interaction is reviewing and promoting — not authoring YAML from scratch.
...
Sidereal references abstract capabilities — not specific tools. Select a deployment profile that matches your cluster's stack, and Sidereal renders the correct admission policies, backend connections, and compliance documentation.
...
|---|---|
| [Engineering Specification](sidereal-engineering-summary.md) | Canonical design document — architecture, probe design, security model, all design decisions |
