...
**Repo**: `primaris-tech/gauntlet` on GitHub (private). Branch protection not yet configured (requires GitHub Team plan or public repo).
...
**Core CRDs** (8 total, API group `gauntlet.io/v1alpha1`):
- `GauntletProbe` — probe configuration; maps to MITRE ATT&CK technique + multi-framework controls; includes `targetNamespace` or `targetNamespaceSelector`, `executionMode` (`dryRun`/`observe`/`enforce`), `intervalSeconds`; supports built-in and `custom` probe types
- `GauntletProbeResult` — append-only audit record; HMAC-verified; TTL per impact level (365d High/Moderate, 180d Low); unified outcome enum + derived `controlEffectiveness` (Effective/Ineffective/Degraded/Compromised); `controlMappings` for multi-framework tagging
- `GauntletIncident` — control failure record; created only in `enforce` execution mode when `controlEffectiveness` is `Ineffective` or `Compromised`; delivers to IR webhook
...
- `GauntletProbeRecommendation` — discovery-generated probe suggestion; `pending`/`promoted`/`dismissed`/`superseded` lifecycle; primary onboarding surface
- `GauntletReport` — (optional) scheduled report generation; continuous monitoring summaries, POA&M, coverage matrices, evidence packages
...
- **Per-Probe ServiceAccounts** — 7 built-in (`gauntlet-controller`, `gauntlet-probe-rbac`, `gauntlet-probe-netpol`, `gauntlet-probe-admission`, `gauntlet-probe-secret`, `gauntlet-probe-detection`, `gauntlet-discovery`) plus operator-registered custom probe ServiceAccounts, each with minimum required RBAC and nothing else
...
- `executionMode: dryRun` is the **default** on fresh install; graduated adoption via `observe` → `enforce` requires `gauntlet-live-executor` role
...
- **Deployment profile abstraction**: Gauntlet references abstract capabilities, not specific tools. Six pre-built profiles ship: `kyverno-cilium-falco`, `opa-calico-tetragon`, `kyverno-eks`, `opa-aks`, `kyverno-gke`, `opa-rke2`. Custom profiles supported.
...
- **Discovery as primary onboarding**: `GauntletProbeRecommendation` CRD with controller-driven discovery — ISSOs review and promote, not author from scratch
