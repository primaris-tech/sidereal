# Sidereal Customer Responsibility Matrix
...
control between Sidereal (the software) and the deploying agency. Agencies use this document
...
| Code | Meaning |
|---|---|
| **G** | Sidereal Implemented — Sidereal fully satisfies this control through its design and operation |
| **S** | Shared — Both Sidereal and the deploying agency share responsibility; see Notes for split |
...
| **N/A** | Not Applicable — This control does not apply to Sidereal's operational context |
...
|---|---|---|---|
| AC-1 | Policy and Procedures | S | Sidereal provides RBAC role definitions and access design; agency writes and enforces access control policy |
| AC-2 | Account Management | S | Sidereal defines roles and requires individual accounts; agency provisions, reviews, and deprovisions accounts per AC-2 procedures |
| AC-3 | Access Enforcement | G | Kubernetes RBAC enforced by Sidereal's role definitions; probe results validate enforcement effectiveness |
| AC-4 | Information Flow Enforcement | G | NetworkPolicy probes continuously validate east-west traffic enforcement |
| AC-5 | Separation of Duties | S | Sidereal provides technically separate roles (operator, approver, live-executor, audit-admin); agency policy must prohibit same individual holding conflicting roles |
...
| AC-11 | Device Lock | N/A | No interactive user sessions in Sidereal's operational context |
...
| AC-16 | Security and Privacy Attributes | S | Sidereal applies probe-id labels, NIST control tags, and multi-framework `controlMappings`; agency manages broader attribute policy |
...
| AC-23 | Data Mining Protection | N/A | Not applicable to Sidereal's operational context |
...
| AT-3 | Role-Based Training | S | Sidereal provides documentation on role responsibilities (CMP, IRP, RoB); agency conducts role-based training for Sidereal operators |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| AU-1 | Policy and Procedures | S | Sidereal provides audit design and AU-12 event enumeration; agency writes audit policy and procedures |
| AU-2 | Event Logging | G | Sidereal defines and generates all audit-relevant events per AU-12 enumeration in the engineering design |
...
| AU-6 | Audit Record Review, Analysis, and Reporting | S | Sidereal exports structured records to SIEM (JSON, CEF, LEEF, Syslog, or OCSF format) and generates continuous monitoring reports via `sidereal report` CLI; agency is responsible for review, analysis, and formal reporting procedures |
| AU-7 | Audit Record Reduction and Report Generation | G | Sidereal provides built-in report generation: continuous monitoring summaries, POA&M generation, coverage matrices, and OSCAL-native evidence packages via `sidereal report` CLI and optional `SiderealReport` CRD |
| AU-8 | Time Stamps | G | All audit records include RFC 3339 timestamps; time synchronization inherited from Kubernetes platform |
| AU-9 | Protection of Audit Information | G | HMAC signing of all results; admission enforcement policy enforces append-only SiderealProbeResult; TLS + payload signing on SIEM export; S3 object lock in COMPLIANCE mode |
...
| AU-11 | Audit Record Retention | S | Sidereal enforces impact-level-dependent in-cluster TTL minimums (365d High/Moderate, 180d Low); agency configures SIEM for long-term retention (3yr High/Moderate, 1yr Low) per their AU-11 policy |
...
| AU-15 | Alternate Audit Logging Capability | G | In-cluster SiderealProbeResult records serve as resilient backup if SIEM is unavailable |
...
| CA-2 | Control Assessments | S | Sidereal produces continuous multi-framework-mapped control evidence with `controlEffectiveness` normalization and automated assessment evidence packages (`sidereal report evidence-package`); agency conducts formal control assessments using Sidereal evidence as primary input |
| CA-3 | Information Exchange | S | Sidereal documents all external connections with direction, data type, protocol, and security controls; agency executes required ISAs with each external system owner |
| CA-5 | Plan of Action and Milestones | S | Sidereal generates SiderealIncident resources from control failures (in `enforce` execution mode) and provides automated POA&M generation via `sidereal report poam`; agency manages formal POA&M process and tracks remediation |
| CA-6 | Authorization | C | AO authorization is the deploying agency's responsibility; detection probes require documented AO authorization referencing SiderealAOAuthorization CRD |
| CA-7 | Continuous Monitoring | G | Core Sidereal capability; continuous probe execution with NIST-mapped results is the continuous monitoring implementation |
| CA-8 | Penetration Testing | S | Sidereal detection probes require AO authorization per CA-8; agency conducts annual formal penetration testing as a separate activity |
| CA-9 | Internal System Connections | S | Sidereal documents internal connections; agency documents full internal connection inventory in SSP |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| CM-1 | Policy and Procedures | S | Sidereal provides CMP; agency writes configuration management policy |
...
| CM-3 | Configuration Change Control | G | Security-relevant Helm value changes require sidereal-security-override role; changes generate audit records exported to SIEM |
| CM-4 | Impact Analysis | S | Agency conducts impact analysis for Sidereal changes; Sidereal documents security-relevant configuration parameters to assist analysis |
| CM-5 | Access Restrictions for Change | G | sidereal-security-override role restricts who can modify security-relevant configuration |
...
| CM-11 | User-Installed Software | N/A | No user-installable software in Sidereal's operational context |
| CM-12 | Information Location | C | Agency documents where Sidereal data resides within their broader information architecture |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| CP-1 | Policy and Procedures | S | Sidereal provides CP document; agency writes contingency planning policy |
...
| CP-3 | Contingency Training | C | Agency trains personnel on Sidereal contingency procedures |
| CP-4 | Contingency Plan Testing | C | Agency conducts contingency plan testing on defined schedule |
| CP-6 | Alternate Storage Site | C | Agency provisions alternate storage for Sidereal data per their CP |
...
| CP-9 | System Backup | S | Sidereal provides Velero-based backup guidance for SiderealProbe configurations; agency executes and validates backups |
| CP-10 | System Recovery and Reconstitution | S | Sidereal provides recovery procedures; agency executes and tests recovery |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| IA-1 | Policy and Procedures | S | Sidereal provides authentication design; agency writes IA policy |
| IA-2 | Identification and Authentication (Organizational Users) | S | Sidereal requires individual (non-shared) accounts; agency provisions and manages identity lifecycle |
...
| IA-4 | Identifier Management | S | Sidereal uses Kubernetes principal names as identifiers; agency manages identifier lifecycle per IA-4 |
| IA-5 | Authenticator Management | S | Sidereal documents credential rotation requirements; agency manages credential lifecycle and rotation for backend integrations |
...
| IA-12 | Identity Proofing | C | Agency conducts identity proofing for Sidereal operators per PS-2 position risk designations |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| IR-1 | Policy and Procedures | S | Sidereal provides IRP template; agency writes incident response policy |
...
| IR-4 | Incident Handling | S | Sidereal generates SiderealIncident resources for control failures when `executionMode: enforce` (incidents are suppressed in `dryRun` and `observe` modes) and exports to configured IR webhook; agency handles incidents per their IRP |
| IR-5 | Incident Monitoring | S | Sidereal tracks control failure incidents via SiderealIncident resources; agency monitors and tracks incidents in their IR system |
| IR-6 | Incident Reporting | S | Sidereal generates incident records with mandatory reporting window configuration; agency executes required US-CERT/CISA reporting |
| IR-7 | Incident Response Assistance | C | Agency provides incident response assistance resources |
| IR-8 | Incident Response Plan | S | Sidereal provides IRP template in compliance/plans/; agency customizes and maintains their IRP |
...
| MA-2 | Controlled Maintenance | S | Agency conducts Sidereal maintenance activities; Sidereal provides upgrade procedures |
...
| MA-6 | Timely Maintenance | S | Sidereal publishes critical patches within 30 days; agency schedules and applies patches |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| MP-1 through MP-8 | All Media Protection controls | C | Agency manages physical and digital media; Sidereal has no physical media |
...
| PL-2 | System Security Plan | S | Sidereal provides pre-written SSP template with control implementations; agency customizes for their environment, personnel, and infrastructure |
| PL-4 | Rules of Behavior | S | Sidereal provides RoB template in compliance/plans/; agency customizes and obtains signatures |
| PL-8 | Security and Privacy Architectures | S | Sidereal provides system boundary, data flow, and architecture documentation; agency incorporates into their enterprise security architecture |
| PL-9 | Central Management | C | Agency manages centralized security control administration |
| PL-10 | Baseline Selection | S | Agency selects and documents applicable NIST 800-53 baseline; Sidereal's `global.impactLevel` setting auto-tunes operational parameters to the selected baseline |
...
| PS-2 | Position Risk Designation | C | Agency designates risk level for positions with Sidereal access; sidereal-operator, sidereal-live-executor, sidereal-approver positions are security-sensitive |
| PS-3 | Personnel Screening | C | Agency screens personnel per PS-2 risk designations |
| PS-4 | Personnel Termination | C | Agency deprovisions Sidereal access on termination per AC-2 procedures |
| PS-5 | Personnel Transfer | C | Agency reviews and adjusts Sidereal access on personnel transfer |
| PS-6 | Access Agreements | C | Agency obtains signed access agreements from all Sidereal operators |
| PS-7 | External Personnel Security | C | Agency applies PS-7 requirements to contractors with Sidereal access |
| PS-8 | Personnel Sanctions | C | Agency manages sanctions for policy violations |
| PS-9 | Position Descriptions | C | Agency includes Sidereal responsibilities in position descriptions |
...
| PT-2 | Authority to Process PII | C | Agency documents legal authority to process any PII in Sidereal audit records |
| PT-3 | Personally Identifiable Information Processing Purposes | S | Sidereal documents what data probe results contain; agency determines PII applicability and documents processing purposes |
...
| RA-2 | Security Categorization | S | Agency categorizes Sidereal per FIPS 199 and sets `global.impactLevel` accordingly; Sidereal cascades appropriate defaults for the selected impact level |
| RA-3 | Risk Assessment | S | Sidereal provides formal risk classifications for each probe type (detection: Medium, others: Low); agency conducts formal system-level risk assessment |
| RA-5 | Vulnerability Monitoring and Scanning | S | Sidereal scans its own dependencies (Dependabot, Grype, Trivy) and publishes results with each release; agency scans running Sidereal deployment per their RA-5 program |
| RA-7 | Risk Response | C | Agency manages risk response decisions |
| RA-9 | Criticality Analysis | C | Agency conducts criticality analysis for their Sidereal deployment |
...
| SA-3 | System Development Life Cycle | S | Sidereal follows a documented SDLC with security integrated throughout; agency manages their acquisition and deployment lifecycle |
...
| SA-9 | External System Services | S | Sidereal documents all external system connections; agency governs external service agreements (ISAs, contracts) |
...
| SA-15 | Development Process, Standards, and Tools | S | Sidereal documents development toolchain; agency verifies toolchain meets their acquisition standards |
| SA-16 | Developer-Provided Training | S | Sidereal provides operator documentation; agency delivers training to operators |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| SC-1 | Policy and Procedures | S | Sidereal provides communications protection design; agency writes SC policy |
...
| SC-4 | Information in Shared System Resources | G | SiderealProbeResult resources are namespace-scoped; results for one namespace not accessible to operators of another |
| SC-5 | Denial-of-Service Protection | S | Sidereal applies ResourceQuota to sidereal-system; agency provides platform-level DoS protection |
| SC-7 | Boundary Protection | G | NetworkPolicy probes continuously validate boundary enforcement; sidereal-system NetworkPolicy enforces explicit egress rules |
...
| SC-17 | Public Key Infrastructure Certificates | S | Sidereal uses certificates for mTLS; agency operates or selects the PKI/CA issuing certificates |
| SC-23 | Session Authenticity | G | mTLS for all backend sessions; probe Job identity verified via pod labels and HMAC |
| SC-24 | Fail in Known State | G | Degraded mode on admission policy absence; SiderealSystemAlert halts execution until acknowledged |
| SC-28 | Protection of Information at Rest | S | Sidereal requires SSE-KMS for S3 and object lock in COMPLIANCE mode; agency configures etcd encryption for in-cluster data at rest |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| SI-1 | Policy and Procedures | S | Sidereal provides integrity design documentation; agency writes SI policy |
...
| SI-3 | Malicious Code Protection | S | Sidereal probe images scanned in CI before signing; agency manages host-level and network-level malicious code protection |
| SI-4 | System Monitoring | G | Core Sidereal capability; continuous active probing with execution jitter (SI-4 timing attack mitigation) and NIST-mapped results |
...
| SI-9 | Information Input Restrictions | G | Admission control probes validate input restriction enforcement; SiderealProbe spec validated at admission |
...
| SI-13 | Predictable Failure Prevention | S | Sidereal provides degraded mode and SiderealSystemAlert; agency monitors and responds to degraded state |
...
| SI-18 | Personally Identifiable Information Quality Operations | S | Sidereal provides redaction configuration options; agency determines PII applicability and enables redaction as needed |
| SI-19 | De-Identification | S | Sidereal provides privacy.redactNamespaceNames and privacy.redactWorkloadIdentities; agency determines when de-identification is required |
...
| Control | Name | Responsibility | Notes |
|---|---|---|---|
| SR-1 | Policy and Procedures | S | Sidereal provides supply chain design documentation; agency writes SCRM policy |
| SR-2 | Supply Chain Risk Management Plan | S | Sidereal documents supply chain controls (signing, SBOM, digest pinning); agency writes formal SCRM plan |
...
| SR-5 | Acquisition Strategies, Tools, and Methods | C | Agency manages acquisition strategy for Sidereal procurement |
| SR-6 | Supplier Assessments and Reviews | C | Agency assesses Sidereal as a supplier per their SCRM program |
...
The controls where Sidereal provides the most complete implementation are:

- **AU-2, AU-3, AU-9, AU-10, AU-12** — Full audit trail with integrity protection
- **CA-7** — Continuous monitoring is Sidereal's core capability
