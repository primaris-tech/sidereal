...
x-trestle-comp-def-rules:
  sidereal:
    - name: fips-140-2-boringcrypto-aws-lc-rs
      description: All Sidereal cryptographic operations use FIPS 140-2 validated modules — BoringCrypto for Go components and aws-lc-rs for Rust components — with no fallback to non-FIPS primitives; FIPS self-tests run at container startup
x-trestle-global:
  profile:
...
For federal systems, this means FIPS 140-2 validated modules. Sidereal
...
5. A `SiderealSystemAlert` is created: `reason: FIPSModuleFailure`
...
CMVP certificate numbers are documented in the Sidereal release notes
...
- `SiderealSystemAlert` CRs for any FIPS self-test failure
...
1. Verify that `fips.enabled: true` is set in the Sidereal Helm values and
...
3. Document the CMVP certificate numbers for BoringCrypto and AWS-LC in
   their SSP IA-7 statement, referencing the specific Sidereal release
4. Treat any `SiderealSystemAlert` with `reason: FIPSModuleFailure` as a
