...
x-trestle-comp-def-rules:
  sidereal:
    - name: sbom-published-per-release
      description: A CycloneDX SBOM is generated and published as a cosign-attested OCI artifact for every Sidereal release, covering all Go and Rust components and their transitive dependencies
x-trestle-global:
  profile:
...
Sidereal's component inventory is a machine-generated, cryptographically
...
The Sidereal CI pipeline generates a CycloneDX SBOM for every release using:
...
The SBOM covers all Sidereal component images:
...
- An agency's assessor can independently verify the inventory of any
  deployed Sidereal version without trusting a separately maintained
  document
...
The `sidereal-image-signature-required` admission enforcement policy (CM-14) blocks
admission of any container image in `sidereal-system` that does not have
a valid cosign signature from the Sidereal release key. This policy
functions as an automated unauthorized component detector: any image not
produced by the Sidereal build pipeline is rejected before it can run.
...
1. Retrieve and review the SBOM for each Sidereal release as part of their
...
2. Integrate the SBOM into their organization's software asset management
   system to satisfy their own CM-8 implementation for the Sidereal
   component
3. Document Sidereal's component inventory (by image digest and chart
