...
x-trestle-comp-def-rules:
  sidereal:
    - name: helm-values-schema-controller-enforcement
      description: Sidereal's Helm chart includes a JSON schema for values validation, and the controller rejects runtime configurations that deviate from the approved settings at admission
x-trestle-global:
  profile:
...
Sidereal enforces secure configuration settings at two layers: pre-deploy
...
The Sidereal Helm chart ships with a `values.schema.json` that enforces
...
| `global.executionMode` | Default `dryRun` on fresh install | Graduated adoption (`dryRun` → `observe` → `enforce`); explicit opt-in required via `sidereal-live-executor` role |
...
OpenAPI v3 schema constraints on `SiderealProbe` and related CRDs at
creation and update time. Configurations rejected at admission include:

- A `SiderealProbe` with `intervalSeconds` exceeding the High-baseline
  maximum without a documented AO-approved deviation
- A `SiderealProbe` targeting `kube-system` without a justification
  annotation
- A detection probe without a `SiderealAOAuthorization` reference
...
Sidereal's default configuration settings represent the most restrictive
mode consistent with operation:

- `global.executionMode: dryRun` — no live probes without explicit opt-in via `sidereal-live-executor` role; graduated path through `observe` then `enforce`
...
produce `SiderealSystemAlert` CRs that suspend probe execution until
...
- `SiderealSystemAlert` CRs for detected configuration drift, exported to
  SIEM
...
1. Document any site-specific deviations from Sidereal's default
...
3. Treat the Sidereal `values.schema.json` as an authoritative reference
