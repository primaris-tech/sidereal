...
x-trestle-comp-def-rules:
  sidereal:
    - name: continuous-probe-evidence-for-assessments
      description: Sidereal produces continuous, timestamped SiderealProbeResult and SiderealIncident records across all five probe surfaces that serve as machine-readable assessment evidence importable into OSCAL Assessment Results
x-trestle-global:
  profile:
...
Sidereal transforms control assessment from a periodic manual event into
...
Sidereal's value: existing tools verify configuration; Sidereal verifies
effectiveness.
...
undetected. Sidereal asks: "is this control enforcing its policy right
...
Every probe execution produces a `SiderealProbeResult` CR with:
...
Each `SiderealProbeResult` includes the specific control IDs being assessed
...
`SiderealProbeResult` data is formatted for export as OSCAL Assessment
...
directly from Sidereal's OSCAL output without manual correlation.

### Assessment Independence

Sidereal's controller is architecturally independent from the controls
...
Sidereal's probe execution is structurally independent of the control
...
For the organizational independence requirement (human assessors), Sidereal's
...
`SiderealProbeResult` records exported to the SIEM are accessible to
...
Sidereal's continuous assessment output to inform their independent review.

## Evidence Produced

- `SiderealProbeResult` CRs for all built-in and custom probe surfaces (append-only,
  HMAC-signed, impact-level-dependent TTL, SIEM-exported with configurable format),
  including `controlEffectiveness` normalization and multi-framework `controlMappings`
- `SiderealIncident` CRs for each detected control failure (in `enforce` execution mode),
  mapped to specific control IDs across all active compliance frameworks
- `SiderealProbeRecommendation` CRs generated by the discovery reconciler,
  documenting control coverage and gaps
- OSCAL Assessment Results generated from `SiderealProbeResult` data via
  `sidereal report evidence-package`, importable into agency ATO tooling
- Continuous monitoring summary reports via `sidereal report continuous-monitoring`
- POA&M entries via `sidereal report poam`
...
1. Review Sidereal's probe scope and scheduling parameters as part of the
   ATO process to confirm that continuous assessment coverage meets their
   Security Assessment Plan requirements
2. Declare the system's FIPS 199 impact level in Sidereal Helm values to
   enforce the correct assessment cadence
3. Designate personnel to triage `SiderealIncident` records within the
   agency's defined response SLA
4. Use Sidereal's OSCAL Assessment Results output as primary evidence in
