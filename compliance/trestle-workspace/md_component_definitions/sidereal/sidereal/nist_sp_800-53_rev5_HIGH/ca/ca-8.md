...
x-trestle-comp-def-rules:
  sidereal:
    - name: detection-probe-requires-ao-authorization
      description: Sidereal detection coverage probes perform adversarial simulation and require a SiderealAOAuthorization CR before execution; probes run in a sandboxed context with no shell, no network, and a restrictive seccomp profile
x-trestle-global:
  profile:
...
Sidereal's Detection Coverage probe surface transforms penetration testing
from a periodic engagement into a continuous automated process. Rather than
hiring a red team once per year, Sidereal emits known-bad syscall patterns
...
against us?" at a point in time. Sidereal's detection probe asks: "would
...
| `Undetected` | No alert raised within the verification window | Fail — detection gap; `SiderealIncident` created |
| `Blocked` | Detection backend enforcement mode (e.g., Tetragon enforcement mode) prevented the syscall | Pass — enforcement active |
| `BackendUnreachable` | Detection backend unavailable | `SiderealSystemAlert` — monitoring halted |
...
not schedule a detection probe Job unless a `SiderealAOAuthorization` CR
is present and valid.

The `SiderealAOAuthorization` CR encodes the rules of engagement:
...
- **Expiry window**: authorization is time-bounded; expired authorizations
  halt probe execution and generate a `SiderealSystemAlert`
...
1. `SiderealProbeResult` CR with `result.outcome: Undetected` and the
   specific MITRE technique that went undetected
2. `SiderealIncident` CR with the detection gap details, exported to SIEM
...
- The `SiderealIncident` CR has a `remediationStatus` field updated by the
...
- Closing a detection gap (e.g., updating a detection rule) is verified by
  ...
  was effective
- The `SiderealProbeResult` record of the first post-fix `Detected` outcome
...
detection backend instances it tests. The probe runner (Sidereal) and the
...
engagements, Sidereal's coverage is continuous — detection gaps are found
...
Sidereal provides the continuous baseline between those engagements.

## Evidence Produced

- `SiderealAOAuthorization` CRs recording the authorizing principal, scope,
  authorization window, and justification for each detection probe campaign
- `SiderealProbeResult` CRs mapping simulated MITRE techniques to detection
  outcomes (Detected / Undetected / Blocked / BackendUnreachable)
- `SiderealIncident` CRs for each `Undetected` outcome with technique
  identifier and detection backend query results, exported to SIEM
- Kubernetes audit log entries for `SiderealAOAuthorization` creation
...
1. Have an authorized ISSO or AO create the `SiderealAOAuthorization` CR
   before detection probes execute, defining the scope, time window, and
   authorized MITRE techniques
2. Review `SiderealIncident` records for `Undetected` outcomes and track
...
4. Conduct periodic full red team exercises by independent human testers
   to supplement Sidereal's continuous detection coverage testing
5. Document Sidereal's detection probe results as penetration testing
