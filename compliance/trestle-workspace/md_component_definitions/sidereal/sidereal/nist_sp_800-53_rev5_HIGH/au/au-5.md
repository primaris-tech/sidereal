...
x-trestle-comp-def-rules:
  sidereal:
    - name: siem-export-failure-alert-and-retry
      description: Sidereal detects SIEM export failures via consecutive failure thresholds, raises SiderealSystemAlert with reason SIEMExportDegraded, buffers undelivered records with exponential-backoff retry, and optionally halts probe scheduling when audit delivery cannot be guaranteed
x-trestle-global:
  profile:
...
An audit processing failure in Sidereal means one of two things: the SIEM
export pipeline cannot durably deliver records off-cluster, or the in-cluster
record creation fails. Sidereal detects both conditions, alerts automatically,
...
`SiderealProbeResult` export attempt. After a configurable consecutive
failure threshold (default: 3), the export channel is marked degraded.

Prometheus metrics exposed for monitoring:
| Metric | Description |
|---|---|
| `sidereal_siem_export_failures_total` | Cumulative count of export failures per channel |
| `sidereal_siem_export_retry_queue_depth` | Records buffered pending retry |
| `sidereal_siem_backend_reachability` | 0/1 gauge per configured SIEM endpoint |
| `sidereal_audit_export_failures` | Rate of export failures (alert threshold metric) |

**In-cluster write failures**: If the controller cannot write a
`SiderealProbeResult` to the Kubernetes API (etcd unavailable, quota
exceeded), the failure is logged and surfaced as a `SiderealSystemAlert`.

### Alert Generation

When the consecutive export failure threshold is reached, the controller
creates a `SiderealSystemAlert` with `reason: SIEMExportDegraded`. This
alert:
...
- Triggers Alertmanager rules configured by the agency against the
  `sidereal_siem_export_failures_total` metric
- Is visible in the controller's structured log output

The `SiderealSystemAlert` creation event is recorded in the Kubernetes
...
- Buffer depth: bounded by the `sidereal-system` namespace ResourceQuota

Records pending delivery are buffered in the controller's in-memory export
queue. The queue depth is reported via `sidereal_siem_export_retry_queue_depth`
...
audit processing has failed — Sidereal stops generating records it cannot
...
| T+0 | Record queued for retry; `sidereal_siem_export_failures_total` incremented |
| T+(threshold failures) | `SiderealSystemAlert` created; `reason: SIEMExportDegraded` |
...
| T+(recovery) | `SiderealSystemAlert` resolved; resolved event exported to SIEM |

### Enhancement: AU-5(1) — Storage Capacity Warning

The `sidereal_audit_record_count` metric provides advance warning of
...
`SiderealSystemAlert` creation triggers the Alertmanager rules configured
...
- `SiderealSystemAlert` CRs with `reason: SIEMExportDegraded`, including
  timestamps for alert onset and resolution, exported to SIEM when
  delivery recovers
- Prometheus metrics: `sidereal_siem_export_failures_total`,
  `sidereal_siem_export_retry_queue_depth`, `sidereal_siem_backend_reachability`
- Controller structured logs recording export failure events, retry
  attempts, and recovery events
- Kubernetes audit log entries for `SiderealSystemAlert` creation and
  resolution (independent notification record)

## Customer Responsibility

The deploying agency must:
1. Configure Alertmanager rules against `sidereal_siem_export_failures_total`
   and `SiderealSystemAlert` resources to notify designated operations staff
...
3. Maintain SIEM endpoint availability and ensure SIEM credentials in
   `sidereal-system` Secrets remain valid (expired credentials are the
   most common cause of export failure)
4. Define and document the agency's response procedure for
   `SiderealSystemAlert` with `reason: SIEMExportDegraded`, including
