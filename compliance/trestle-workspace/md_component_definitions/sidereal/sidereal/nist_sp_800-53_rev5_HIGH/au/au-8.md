...
x-trestle-comp-def-rules:
  sidereal:
    - name: rfc3339-timestamps-all-probe-results
      description: All SiderealProbeResult, SiderealIncident, and SiderealSystemAlert records include RFC 3339 UTC timestamps with nanosecond precision, sourced from NTP-synchronized node clocks and preserved through SIEM export
x-trestle-global:
  profile:
...
Every Sidereal audit record carries RFC 3339 UTC timestamps with nanosecond
...
All timestamps in Sidereal audit records use RFC 3339 format with full
...
|---|---|
| `SiderealProbeResult` | `probeStartTime`, `probeEndTime`, `resultWriteTime`, `siemExportTime` |
| `SiderealIncident` | `detectedAt`, `reportedAt`, `webhookDeliveredAt` |
| `SiderealSystemAlert` | `raisedAt`, `acknowledgedAt`, `resolvedAt` |
| `SiderealAOAuthorization` | `authorizedAt`, `expiresAt` |
...
`probeStartTime` of the originating `SiderealProbeResult` — not the
delivery time.
...
The `siemExportTime` field in the `SiderealProbeResult` records the actual
...
The accuracy of all Sidereal timestamps depends on the node clock being
...
The Kubernetes nodes running Sidereal must synchronize to an authoritative
NTP time source. Sidereal's audit records are only as accurate as the node
...
- `SiderealProbeResult` CRs with RFC 3339 UTC `probeStartTime`,
  `probeEndTime`, `resultWriteTime`, and `siemExportTime` fields
- `SiderealIncident` CRs with `detectedAt` and `reportedAt` timestamps
- `SiderealSystemAlert` CRs with `raisedAt`, `acknowledgedAt`, and
...
3. Document the NTP configuration for Sidereal's hosting nodes in the SSP
   as a platform-level implementation of AU-8(1)
4. Not adjust node system clocks manually or through non-NTP mechanisms
   while Sidereal is generating audit records, as this would corrupt the
