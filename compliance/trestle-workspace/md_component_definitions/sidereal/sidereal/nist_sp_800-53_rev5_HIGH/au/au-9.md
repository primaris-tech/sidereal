...
x-trestle-comp-def-rules:
  sidereal:
    - name: sidereal-audit-protection
      description: Sidereal protects audit information from unauthorized access, modification, and deletion through cryptographic and admission enforcement controls
x-trestle-global:
  profile:
...
`SiderealProbeResult` resources for all principals, including cluster-admin. Only CREATE is permitted. This is enforced at the Kubernetes admission
layer — independent of the Sidereal controller. A compromised controller cannot
...
certificate validation. Audit payloads are signed with the Sidereal signing key before
...
The `sidereal-system` NetworkPolicy restricts all egress from probe Jobs. The result
...
In-cluster `SiderealProbeResult` records and SIEM-exported records are maintained
...
Payload signing with the Sidereal signing key (HSM/KMS-backed, SC-12) provides
cryptographic protection of records in transit.

### Enhancement: AU-9(4) — Access by Subset of Privileged Users

The `sidereal-audit-admin` role provides read-only access to `SiderealProbeResult`
...
- Admission enforcement policy denying UPDATE/DELETE on SiderealProbeResult (shipped in Helm chart)
- HMAC verification status field in every SiderealProbeResult record
...
1. Assign the `sidereal-audit-admin` and cluster-admin roles to separate individuals per AU-9(4)
2. Configure their SIEM to verify Sidereal payload signatures on ingest
