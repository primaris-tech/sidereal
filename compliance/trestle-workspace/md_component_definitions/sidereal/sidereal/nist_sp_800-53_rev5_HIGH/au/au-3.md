...
x-trestle-comp-def-rules:
  sidereal:
    - name: sidereal-audit-record-content
      description: Sidereal audit records contain all required content fields per AU-3
x-trestle-global:
  profile:
...
Every `SiderealProbeResult` record contains the following fields satisfying all AU-3
content requirements:

| AU-3 Requirement | Sidereal Field | Description |
|---|---|---|
...
`SiderealProbeResult` records additionally include:
- `probe.aoAuthorizationRef` — for Detection probes, the SiderealAOAuthorization resource
...
- `SiderealProbeResult` CRD schema (published in Helm chart, defines all required fields)
...
ingest pipelines if their AU-3 policy requires additional content beyond what Sidereal
provides.
