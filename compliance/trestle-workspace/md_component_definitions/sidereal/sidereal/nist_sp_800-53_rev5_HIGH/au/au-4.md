...
Sidereal implements a two-tier audit record storage architecture. The
...
`SiderealProbeResult` CRs are stored in the Kubernetes API server (etcd).
...
on all `SiderealProbeResult` CRs at admission time (365 days for High/Moderate, 180 days
...
DELETE operations on `SiderealProbeResult` resources (SR-9) ensures that
...
**Capacity monitoring**: The `sidereal_audit_record_count` Prometheus metric
tracks the current number of `SiderealProbeResult` records in the cluster.
...
**ResourceQuota bounding**: The `sidereal-system` namespace ResourceQuota
...
Every `SiderealProbeResult` and `SiderealIncident` record is exported to
...
Sidereal is designed to never silently fail to record an audit event due
to capacity pressure:
- **Pre-failure alerting**: `sidereal_audit_record_count` and the namespace
...
The SIEM export is Sidereal's implementation of transfer to alternate
...
- `SiderealProbeResult` CRs in etcd with controller-enforced 365-day
...
- `sidereal_audit_record_count` Prometheus metric time series
- `sidereal_audit_export_failures` Prometheus metric (capacity pressure
  early warning)
...
2. Configure S3 Object Lock in COMPLIANCE mode (not just GOVERNANCE mode)
   for the Sidereal S3 export bucket if using S3 as the SIEM target
3. Configure Alertmanager rules for `sidereal_audit_record_count` and
   `sidereal_audit_export_failures` to alert before capacity thresholds
   are breached
4. Ensure SIEM credentials stored in `sidereal-system` Secrets are rotated
