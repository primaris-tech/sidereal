...
x-trestle-comp-def-rules:
  sidereal:
    - name: cosign-sbom-digest-pinning-admission-supply-chain-controls
      description: Sidereal implements supply chain controls via cosign image signing, SBOM attestation, SHA-256 digest pinning in the Helm chart, and admission enforcement of signature verification
x-trestle-global:
  profile:
...
Sidereal's supply chain risk management is implemented as a set of automated
...
All container image references in the Sidereal Helm chart are pinned by
...
The `sidereal-image-signature-required` admission enforcement policy, shipped in
the Sidereal Helm chart, verifies the cosign signature of every image before
any Pod in `sidereal-system` is admitted. This policy enforces that only
...
The admission enforcement policy is deployed before Sidereal itself (Helm pre-install
...
The CI pipeline itself is a supply chain component. Sidereal's pipeline
enforces:
...
- Admission controller policy enforcement records for all Pod deployments in `sidereal-system`
...
1. Integrate Sidereal's published SBOM into their organization's software
   supply chain risk management program (SCRM)
2. Validate cosign signatures before deploying any Sidereal update, using
   the public key distributed with the Helm chart
3. Ensure the admission controller (e.g., Kyverno or OPA/Gatekeeper) is deployed in the cluster before Sidereal installation
   so that admission enforcement is active from the first deployment
4. Treat any admission controller denial for a Sidereal image as a potential
