...
x-trestle-comp-def-rules:
  sidereal:
    - name: sbom-cosign-signature-chain-provenance
      description: Sidereal establishes component provenance through cosign-attested CycloneDX SBOMs and SLSA provenance attestations that link each deployed image digest to its build pipeline, source commit, and dependency tree
x-trestle-global:
  profile:
...
Sidereal's provenance chain links every deployed binary back to a specific
...
For any deployed Sidereal image, the complete provenance chain is:
...
is running in this Sidereal deployment?" without trusting a separately
...
This allows the agency's AO to independently verify that a deployed Sidereal
...
- External to Sidereal's own infrastructure
...
trusting Sidereal as the sole custodian of the provenance record.
...
The digest is not assigned by Sidereal — it is derived from the image
...
image from the Sidereal release pipeline is genuine. An unsigned image, an
...
1. Verify the cosign signature and SBOM attestation for each Sidereal image
   before deploying a new version; store the verification output as part of
   the change management record
2. Include Sidereal component provenance records (image digest, SLSA
...
3. Query the SLSA provenance attestation as part of their supply chain
   review process for each Sidereal upgrade
