...
x-trestle-comp-def-rules:
  gauntlet:
    - name: rbac-probe-validates-access-enforcement
      description: Gauntlet RBAC probe continuously validates that Kubernetes RBAC policies deny unauthorized access as configured, and that authorized access is not inadvertently blocked
x-trestle-global:
  profile:
...
Gauntlet contributes to access enforcement through two complementary
...
### Gauntlet's Internal Access Enforcement

Gauntlet enforces access control for its own resources through layered
Kubernetes RBAC:

**Controller ServiceAccount** — Can perform:
- Create/delete `Job` resources in `gauntlet-system`
- Read/write `GauntletProbe`, `GauntletProbeResult`, `GauntletIncident`,
  `GauntletSystemAlert` CRDs
- Read Secrets in `gauntlet-system` (HMAC root secret only)
- Cannot perform: any operation in non-gauntlet namespaces; any operation
  the probe ServiceAccounts perform

**Per-probe ServiceAccounts** — Each scoped to only what its probe requires:
- `gauntlet-probe-rbac`: reads ClusterRoles/RoleBindings (read-only, cluster-scoped)
- `gauntlet-probe-netpol`: reads NetworkPolicy objects (read-only, target namespace)
- `gauntlet-probe-admission`: creates/deletes test resource specs (specific
  resource types, specific namespaces)
- `gauntlet-probe-secret`: attempts GET on Secrets (scoped to test targets only)
- `gauntlet-probe-detection`: no Kubernetes API access (syscall-only operation)
...
Gauntlet's internal access model is entirely role-based. Every access
...
and no cluster-admin assignments for any Gauntlet component.

### Enhancement: AC-3(2) — Dual Authorization

Security-relevant Gauntlet configuration changes require the
`gauntlet-security-override` ClusterRole (a separate, named authorization)
...
- `GauntletProbeResult` CRs capturing pass/fail for each RBAC and Secret
  Access check (append-only, HMAC-signed, continuous)
- `GauntletIncident` CRs for any detected RBAC misconfiguration or access
  policy violation, with specific API operation and target resource
- Kubernetes audit log entries for all API operations by Gauntlet
...
1. Define and maintain the ClusterRole and RoleBinding manifests that
   express their access control policy — Gauntlet validates enforcement
   of those policies but does not author them
2. Review and approve the ServiceAccount RBAC grants shipped with Gauntlet
   during the ATO process
3. Not grant additional cluster-wide roles to `gauntlet-system`
   ServiceAccounts outside the defined Helm values
4. Configure `GauntletProbe` resources to cover the RBAC boundaries most
