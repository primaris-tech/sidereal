...
x-trestle-comp-def-rules:
  gauntlet:
    - name: no-debug-endpoints-no-shell-in-probe-runners
      description: Gauntlet probe runner images contain no shell, no debug endpoints, and no package manager; only the compiled Rust binary and its minimal runtime dependencies are present
x-trestle-global:
  profile:
...
Least functionality is enforced at every layer of the Gauntlet stack: container
...
A `NetworkPolicy` for the `gauntlet-system` namespace restricts egress to
...
egress restrictions are enforced (a GauntletProbeResult documents each
...
Every Gauntlet probe runner Job enforces:
...
The Gauntlet SBOM (CM-8) is reviewed on every release to confirm that no
...
software whitelist: only images signed by the Gauntlet release key are
admitted into the `gauntlet-system` namespace. No unsigned or externally
sourced image can run as a Gauntlet component, preventing unauthorized
software from executing under the Gauntlet identity.

## Evidence Produced

- Container image SBOM listing only the declared runtime components
  (verifiable via `cosign verify-attestation --type cyclonedx`)
- `GauntletProbeResult` CRs from the NetworkPolicy probe confirming that
...
1. Not add sidecar containers or additional init containers to
   `gauntlet-system` Pods that introduce shell access, debug functionality,
   or network connectivity not present in the baseline deployment
2. Review the SBOM for each Gauntlet release to confirm that no new
   capabilities have been introduced without authorization
3. Not create exceptions to the `gauntlet-system` NetworkPolicy without
...
4. Apply the Kubernetes Pod Security admission policy at `restricted` level
   for the `gauntlet-system` namespace
