...
x-trestle-comp-def-rules:
  gauntlet:
    - name: security-override-role-required-for-config-changes
      description: Modifications to security-relevant Gauntlet configuration require the gauntlet-security-override role; all such changes are recorded in the Kubernetes audit log and SIEM
x-trestle-global:
  profile:
...
Gauntlet enforces configuration change control through layered technical
...
Security-relevant Gauntlet resources are protected by the
`gauntlet-security-override` ClusterRole. This role is required to modify:

- `GauntletProbe` resources (probe schedules, targets, FIPS 199 level)
- The Gauntlet controller ConfigMap (SIEM endpoints, retention TTLs)
- Per-probe ServiceAccount RBAC bindings
- `GauntletAOAuthorization` resources for detection probe authorization
- The `gauntlet-image-signature-required` admission enforcement policy

Any attempt to modify these resources by a principal not holding
`gauntlet-security-override` is denied by Kubernetes RBAC and produces an
...
The `gauntlet-security-override` role is not granted to the Gauntlet
...
An admission enforcement policy enforces that changes to security-critical Gauntlet
...
- A designated GitOps ServiceAccount (e.g., ArgoCD, Flux) that has been
  granted `gauntlet-security-override` by the agency
...
`gauntlet-security-override` ClusterRole. This enforces that all changes
flow through the documented pathway and are traceable to a specific
deployment pipeline run.

Policy violations generate `GauntletIncident` CRs with
...
1. A versioned Helm release Secret in `gauntlet-system` (Helm's native
...
- Prohibition: The admission controller and RBAC deny unauthorized change attempts without
  human intervention
- Notification: `GauntletIncident` CRs for blocked change attempts trigger
  the agency's IR webhook automatically

### Enhancement: CM-3(2) — Test, Validate, and Document Changes

Gauntlet's CI pipeline requires all proposed configuration changes to pass:
...
schedules, signing key references) require `gauntlet-security-override` and
...
- Kubernetes audit log entries for all mutations to `gauntlet-system`
  resources, exported to SIEM (who changed what and when)
- Helm release history Secrets in `gauntlet-system` (before/after
  configuration state for each change)
- `GauntletIncident` CRs for change attempts blocked by the admission controller or RBAC
...
1. Define a formal change control process that includes security and privacy
   impact analysis for Gauntlet configuration changes before approval
2. Restrict binding of the `gauntlet-security-override` ClusterRole to
...
3. Operate the GitOps deployment pathway (ArgoCD, Flux, or equivalent) as
   the exclusive mechanism for applying approved Gauntlet configuration
   changes
4. Designate a change control authority (change control board or ISSO
   equivalent) to approve Gauntlet configuration changes before deployment
