...
x-trestle-comp-def-rules:
  gauntlet:
    - name: helm-values-as-versioned-baseline-configuration
      description: Gauntlet's Helm chart values file constitutes the versioned baseline configuration; the controller enforces that the running state matches the declared values
x-trestle-global:
  profile:
...
Gauntlet's baseline configuration is a first-class artifact managed by the
...
The Gauntlet Helm chart `values.yaml` is the authoritative baseline
...
- ResourceQuota limits for the `gauntlet-system` namespace
...
- GauntletAOAuthorization references for detection probes

Each Gauntlet release is tagged with a semantic version (e.g., `v1.2.0`) and
a corresponding Helm chart version. The chart is published as an OCI artifact
to the Gauntlet container registry, providing a durable, immutable reference
...
Resources in the `gauntlet-system` namespace that were not created by the
...
Helm stores each release as a Kubernetes Secret in the `gauntlet-system`
...
`helm history gauntlet` and rolled back via `helm rollback`. The Kubernetes
...
- `GauntletSystemAlert` CRs for any detected configuration drift from the
  Helm baseline, exported to SIEM
- Helm release history Secrets in `gauntlet-system` (previous configuration
  retention)
...
3. Maintain a Velero or equivalent backup of all Gauntlet CRDs as documented
   in the contingency plan, to support restoration to a known baseline state
4. Document Gauntlet's version and configuration baseline in their SSP CM-2
