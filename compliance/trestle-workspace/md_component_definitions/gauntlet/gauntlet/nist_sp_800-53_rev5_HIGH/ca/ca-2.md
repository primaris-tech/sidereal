...
- `GauntletProbeResult` CRs for all built-in and custom probe surfaces (append-only,
...
- `GauntletIncident` CRs for each detected control failure (in `enforce` execution mode),
  mapped to specific control IDs across all active compliance frameworks
- `GauntletProbeRecommendation` CRs generated by the discovery reconciler,
  documenting control coverage and gaps
- OSCAL Assessment Results generated from `GauntletProbeResult` data via
  `gauntlet report evidence-package`, importable into agency ATO tooling
- Continuous monitoring summary reports via `gauntlet report continuous-monitoring`
- POA&M entries via `gauntlet report poam`
