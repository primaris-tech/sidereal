...
x-trestle-comp-def-rules:
  gauntlet:
    - name: tls12-mtls-fips-cipher-suites-all-transmissions
      description: All Gauntlet data transmissions use TLS 1.2 or higher with FIPS-approved cipher suites enforced by BoringCrypto (Go) and aws-lc-rs (Rust); mTLS is required for all external connections
x-trestle-global:
  profile:
...
Every Gauntlet transmission path — internal to the cluster and external to
...
- `TamperedResult` outcome records in `GauntletProbeResult` CRs when HMAC
  verification fails
...
3. Provide a valid TLS CA bundle for each SIEM endpoint in the Gauntlet
