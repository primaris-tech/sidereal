...
x-trestle-comp-def-rules:
  gauntlet:
    - name: networkpolicy-probe-validates-boundary-enforcement
      description: Gauntlet's NetworkPolicy probe surface actively tests that cluster boundary protections (CNI-enforced NetworkPolicies) block unauthorized cross-namespace and external traffic as declared
x-trestle-global:
  profile:
...
Gauntlet contributes to boundary protection through two distinct mechanisms:
...
The NetworkPolicy probe surface is Gauntlet's direct implementation of SC-7
...
produces a `GauntletIncident` CR with the source namespace, destination,
...
- Originate from `gauntlet-system` but do not match any scheduled probe
  (unfingerpinted)
- Cross a namespace boundary that should be deny-all
- Target an external address not in the permitted egress list

generate a `GauntletSystemAlert` with `reason: UnexpectedNetworkFlow`.
...
### Gauntlet's Own Boundary Implementation

The `gauntlet-system` namespace is a reference implementation of boundary
...
against `gauntlet-system` on each execution cycle, producing
`GauntletProbeResult` records that continuously attest to the boundary
posture of the monitoring system itself.

### Enhancement: SC-7(3) — Access Points

Gauntlet has no publicly accessible endpoints. The controller's HTTP server
...
The `gauntlet-system` NetworkPolicy implements default-deny for both
...
All Gauntlet external communications (SIEM export, CNI observability API
queries) are initiated from the controller's single managed network
interface within `gauntlet-system`. No probe runner establishes external
...
- `GauntletProbeResult` CRs for each tested network flow path, with
  allow/deny outcome and CNI flow verdict source (continuous)
- `GauntletIncident` CRs for boundary enforcement failures, with source
  namespace, destination, and protocol
- `GauntletSystemAlert` CRs for unexpected flows detected by CNI
...
2. Author a complete boundary policy (NetworkPolicy manifests) for all
   namespaces — Gauntlet validates enforcement of declared policy but cannot
   substitute for the agency's responsibility to declare the correct policy
3. Configure `GauntletProbe` resources to test all boundary-relevant flow
