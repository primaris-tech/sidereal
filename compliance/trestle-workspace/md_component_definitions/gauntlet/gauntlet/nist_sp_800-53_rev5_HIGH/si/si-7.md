...
x-trestle-comp-def-rules:
  gauntlet:
    - name: gauntlet-integrity-verification
      description: Gauntlet verifies software, firmware, and information integrity through HMAC result signing, cosign image verification, and append-only audit enforcement
x-trestle-global:
  profile:
...
Gauntlet applies integrity verification at three distinct layers: software
...
This verification is performed by the admission controller independently of the Gauntlet
...
`GauntletProbeResult` resources are protected by an admission enforcement policy
...
`TamperedResult` outcomes automatically create `GauntletSystemAlert` resources
...
- Admission enforcement policy for append-only GauntletProbeResult (shipped in Helm chart)
- HMAC verification status field in every GauntletProbeResult
- TamperedResult outcome records in SIEM when signature verification fails
- GauntletSystemAlert records for integrity violations
...
2. Acknowledge GauntletSystemAlert records created by integrity violations
