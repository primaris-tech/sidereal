...
x-trestle-comp-def-rules:
  gauntlet:
    - name: mtls-all-external-connections
      description: Gauntlet enforces mutual TLS for all connections to external systems including detection backend gRPC APIs, CNI observability APIs, and SIEM endpoints; unauthenticated connections are refused before any data exchange occurs
x-trestle-global:
  profile:
...
In Gauntlet's architecture, "devices" are the external system endpoints the
...
- **Gauntlet authenticates the remote device**: the controller validates the
...
- **The remote device authenticates Gauntlet**: the controller presents its
...
2. A `GauntletSystemAlert` is created with `reason: ExternalAuthFailure`
...
SPIFFE/SPIRE provides automated workload identity for all Gauntlet
components. Each component receives a unique SPIFFE URI as its device
identity (`spiffe://<trust-domain>/ns/gauntlet-system/sa/<component>`).
...
mTLS is bidirectional by definition. Both the controller (Gauntlet's device)
...
- `GauntletSystemAlert` CRs for any external connection that fails mTLS
...
2. Provide the corresponding CA bundle to Gauntlet via Helm values
...
4. For IL4/IL5: deploy and operate SPIFFE/SPIRE for automated SVID-based
   device identity management across all Gauntlet-connected systems
