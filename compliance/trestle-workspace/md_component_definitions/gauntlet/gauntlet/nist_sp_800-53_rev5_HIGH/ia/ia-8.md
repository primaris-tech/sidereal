...
x-trestle-comp-def-rules:
  gauntlet:
    - name: mtls-non-organizational-external-authentication
      description: Gauntlet requires mutual TLS for all connections to non-organizational external systems; server certificates are validated against a configured CA bundle before any data exchange; tlsInsecureSkipVerify cannot be enabled
x-trestle-global:
  profile:
...
In Gauntlet's architecture, non-organizational users are the external
...
be cryptographically identified before Gauntlet delivers probe data or
accepts responses.

IA-8 is the inverse framing from IA-3: where IA-3 focuses on Gauntlet
authenticating external devices, IA-8 focuses on external systems
authenticating themselves to Gauntlet. Both directions are enforced by
...
is sent. The failed authentication event generates a `GauntletSystemAlert`
and SIEM export.
...
cloud-hosted Elasticsearch service). Gauntlet applies layered
authentication:
...
certificate validation. In Gauntlet:
...
Gauntlet accepts X.509 certificates issued by any CA configured in the
`tlsCABundle` Helm value. The agency controls which CAs are trusted —
Gauntlet does not maintain a hardcoded list of trusted CAs. This allows
...
- `GauntletSystemAlert` CRs for any external endpoint that fails mTLS
...
1. Configure the CA bundle for all external system certificates in the
   Gauntlet Helm values; this bundle is the trust anchor for non-org
...
4. Document the external systems Gauntlet connects to and their
